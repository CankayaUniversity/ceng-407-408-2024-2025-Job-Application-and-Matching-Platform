...
export default function BlogPage() {
  ...
  const handleAddComment = (postId) => {
    ...
    axios.post(`http://localhost:9090/blog/comment/${postId}`, {
      content: commentText,
      avatar: 'https://i.pravatar.cc/150?u=you'  // avatar bilgisi buraya eklendi
    }, {
      headers: { Authorization: `Bearer ${token}` }
    })

        .then(() => {
          return axios.get('http://localhost:9090/blog/getAll', {
            headers: { Authorization: `Bearer ${token}` }
          });
        })
        .then((res) => {
  ...
  const handleLike = (postId) => {
    axios.post(`http://localhost:9090/blog/like/${postId}`, {}, {
      headers: { Authorization: `Bearer ${token}` }
    })
        .then(() => {
          return axios.get('http://localhost:9090/blog/getAll', {
                ...
                {(expandedComments[post.id]
                    ...
                    <div key={cmt.id} className="flex items-start gap-2 p-1.5 border border-gray-200 rounded-md">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[18px]">{cmt.content}</span>
                      </div>
                    </div>
                ))}
